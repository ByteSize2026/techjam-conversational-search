...
- [ ] 若修改 `starter/shopping_agent/graph.py`/`llm_nodes.py`，检查 Router 是否仍然是纯代码（不调用模型、不决定事实），Value Node 是否仍然只产生一个事实（不决定下一节点）；新增环必须有有界计数器守卫。

## 专题链接

- [接口、状态与输出规则](./contract-and-state.md)
- [Router / Value-Node 图](./router-value-node-graph.md)
- [有界工具循环（已废弃，历史参考）](./tool-loop.md)
...
- [ ] LLM Value Node 不可用或结构化输出解析失败两次时，默认 deterministic fallback 路径仍可离线运行且旧 evaluator 指标可对照。

...
结论先说：**holdout 上我们略好于同学（0.8888 vs 0.8863），但两边公开集分数都虚高。不要新建知识库（内存目录索引已经是检索库）。下一步不要再刷公开 200；优先硬池 IDF/BM25。细节见 `report/optimize_kb.md`。Holdout ≠ 官方私有 800。**

## 1. 实测对照（公开 200 vs ID-holdout 200）

| 行 | Hit@10 | MRR | MTTC | Efficiency | 技术分 |
|---|---:|---:|---:|---:|---:|
| 我们 × 公开 200 | 1.000 | 0.9467 | 2.530 | 0.847 | **0.9534** |
| 我们 × holdout 200 | 0.980 | 0.7743 | 2.675 | 0.8325 | **0.8888** |
...
- **Holdout 更好的 Agent：我们**（0.8888 > 0.8863）。
- 公开集相对 holdout：**两边都过拟合/分布偏移**。我们 0.953→0.889，同学 0.945→0.886。主因仍是 **MRR**（0.947→0.774）。Hit 现为 0.980（漏 4 条：0005/0052/0135/0183）。`0122` 已用泛约束跳过 MiniLM + 热度锁修好；`0090`（rubber sole）仍走 MiniLM，rank 10 命中。
...
当前 PUBLIC：`gate_size=5`、逐字 AND、`dump_slots=4` / `dump_pool_cap=80`、`min_slots_to_recommend=3`、`w_title=0`、`w_dense=0.1`、`dense_skip_generic=True`（cotton/imported/color 跳过 MiniLM，并锁热度 Top-10；`rubber sole` 这类区分项仍走 MiniLM）。缺 MiniLM 权重则 dense 分量为 0。
...
技术分 0.50×Hit + 0.30×MRR + 0.20×Efficiency。holdout 上从 0.952 掉到 0.888，大约：

- Hit 1.000→0.975：约 −0.0125 分（5/200 miss，权重 0.50）
- MRR 0.943→0.782：约 −0.048 分
- Efficiency 小幅变差
...
硬合取之后的克隆排序在新 ID 上明显变差（我们 holdout MRR 0.782，同学 structured 0.806——他们 holdout 排序略好，我们靠 Hit 和 MTTC 把总分略拉开）。
...
- 下一轮代码改动的验收：**holdout 技术分 > 0.8888 且 Hit ≥ 0.980，同时公开 Hit 不塌。** 只涨公开分、holdout 不动或变差，视为过拟合，不进 PUBLIC。
